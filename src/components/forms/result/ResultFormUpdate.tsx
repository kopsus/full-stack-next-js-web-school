...
export default function ButtonUpdateResult({
  classes,
  grades,
  students,
  subjects,
  ...
  useEffect(() => {
    const grade = grades.find(
      (g) => g.level === initialData.student.grade.level
    );
    setSelectedGrade(grade || null);
    setSelectedClass(
      classes.find((c) => c.id === initialData.student.classId) || null
    );
    setSelectedSubject(
      subjects.find(
        (s) =>
          ...
          (initialData.assignment
            ? initialData.assignment.lesson.subjectId
            : initialData.exam?.lesson.subjectId)
      ) || null
    );
    setSelectedAssessmentType(initialData.assignmentId ? "assignment" : "exam");
    setSelectedLesson(
      initialData.assignment
        ? initialData.assignment.lesson
        : initialData.exam?.lesson || null
    );
  ...
  useEffect(() => {
    if (selectedAssessmentType === "assignment") {
      setFilteredAssignments(
        assignments.filter((a) => a.lessonId === selectedLesson?.id)
      );
      setFilteredExams([]);
  ...
  }, [selectedAssessmentType, selectedLesson, assignments, exams, form]);
                          ...
                          <SelectValue>
                            {
                              filteredStudents.find(
                                (s) =>
                                  s.id.toString() === form.watch("studentId")
                              )?.first_name
                            }{" "}
                            {
                              filteredStudents.find(
                  ...
                  <div>
                    ...
                    <FormControl>
                      <Select
                        value={
                          selectedAssessmentType === "assignment"
                            ? form.watch("assignmentId")?.toString()
                            : form.watch("examId")?.toString()
                        }
