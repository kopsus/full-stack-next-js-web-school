...
import { Student, Teacher, Lesson, Role, Present } from "@prisma/client";
import {
  Select,
  SelectContent,
  SelectItem,
...
export default function ButtonUpdateAttendance({
  allStudents,
  allTeachers,
  allLessons,
  attendance,
  roleLogin,
}: ButtonUpdateAttendanceProps) {
  ...
  const form = useForm<AttendanceSchema>({
    ...
    defaultValues: {
      ...
      information: attendance.information ?? undefined,
    },
  });

  const [selectedType, setSelectedType] = useState<"teacher" | "student">(
    roleLogin === "ADMIN" ? "teacher" : "student"
  );
  const [selectedTeacher, setSelectedTeacher] = useState("");
  const [selectedStudent, setSelectedStudent] = useState("");
  const [selectedLesson, setSelectedLesson] = useState("");
  const [filteredStudents, setFilteredStudents] =
    useState<Student[]>(allStudents);
  ...
  useEffect(() => {
    if (selectedType === "student") {
      const selectedStudentData = filteredStudents.find(
        (s) => s.first_name + " " + s.last_name === selectedStudent
      );
      if (selectedStudentData) {
        setFilteredLessons(
          allLessons.filter(
            (lesson) => lesson.classId === selectedStudentData.classId
          )
        );
      }
    } else if (selectedType === "teacher") {
      const teacherId = allTeachers.find(
        (t) => t.first_name + " " + t.last_name === selectedTeacher
      )?.id;
      setFilteredLessons(
        allLessons.filter((lesson) => lesson.teacherId === teacherId)
      );
    } else {
  ...
  useEffect(() => {
    setSelectedTeacher(
      attendance.teacherId
        ? `${
            allTeachers.find((t) => t.id === attendance.teacherId)?.first_name
          } ${
            allTeachers.find((t) => t.id === attendance.teacherId)?.last_name
          }`
        : ""
    );
    setSelectedStudent(
      attendance.studentId
        ? `${
            allStudents.find((s) => s.id === attendance.studentId)?.first_name
          } ${
            allStudents.find((s) => s.id === attendance.studentId)?.last_name
          }`
        : ""
    );
    setSelectedLesson(
      attendance.lessonId
        ? allLessons.find((l) => l.id === attendance.lessonId)?.name || ""
        : ""
    );
  ...
  async function onSubmit(values: AttendanceSchema) {
    const formattedValues = {
      ...
      present: values.present as Present,
  ...
  const handleTeacherSelect = (currentValue: string) => {
    ...
    setFilteredLessons(
      allLessons.filter(
        (lesson) => lesson.teacherId === selectedTeacherData?.id
      )
    );
    setSelectedLesson("");
  };
      ...
      <AlertDialogContent className="max-w-4xl">
        <AlertDialogHeader>
          ...
          <AlertDialogDescription className="hidden">
            Isi formulir berikut untuk mengedit kehadiran. Pastikan semua data
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-left mb-2">
                  ...
                  {roleLogin === "ADMIN" && (
                    <div className="flex flex-col gap-2 mt-[6px]">
                      ...
                      <Select
                        value={selectedType}
                        onValueChange={(value) =>
                          setSelectedType(value as "teacher" | "student")
                        }
                      >
                        <SelectTrigger>
                  ...
                  <div className="flex flex-col gap-2">
                    <Label>
                      {selectedType === "teacher" ? "Nama Guru" : "Nama Siswa"}
                    </Label>
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Textarea
                              {...field}
                              placeholder="Berikan keterengan"
                              value={field.value}
                            />
                          </FormControl>
                          <FormMessage />
