...
import { Assignment, Lesson, Subject, Teacher, Class, Student, Attendance, Role } from "@prisma/client";
...
export const columns: ColumnDef<AttendanceWithDetails>[] = [
    ...
    },
    {
        ...
        cell: ({ row }) => (
            <div className="flex flex-col min-w-0">
                <p className={`text-xs md:text-sm ${row.original.attendance.present ? "text-green-500" : "text-red-500"}`}>
                    {row.original.attendance.present ? "Hadir" : "Tidak Hadir"}
                </p>
            </div>
        ),
    },
        ...
        cell: ({ row }) => (
            <div className="flex flex-col min-w-0">
                <p className="text-xs md:text-sm truncate">
                    {row.original.attendance.student ? `${row.original.attendance.student.first_name} ${row.original.attendance.student.last_name}` : `${row.original.attendance.teacher?.first_name} ${row.original.attendance.teacher?.last_name}`}
                </p>
            </div>
        ),
    },
            ...
            <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-44">
                    ...
                    {(row.original.roleLogin === "ADMIN" || row.original.roleLogin === "TEACHER") && (
                        <>
                            <ButtonUpdateAttendance allStudents={row.original.allStudents} allTeachers={row.original.allTeachers} allLessons={row.original.allLessons} attendance={row.original.attendance} roleLogin={row.original.roleLogin} />
                    ...
                    <AlertDialogDescription asChild>
                        <div className="mt-4 space-y-4 text-left">
                            <div>
                                ...
                                <span>{dayjs(data.attendance.date).format('dddd, DD MMMM YYYY')}</span>
                            </div>
                            <div>
                            ...
                            <div>
                                <span className="font-semibold block">Nama {data.attendance.teacherId ? "Guru" : "Siswa"}</span>
                                <span>{data.attendance.student ? `${data.attendance.student.first_name} ${data.attendance.student.last_name}` : `${data.attendance.teacher?.first_name} ${data.attendance.teacher?.last_name}`}</span>
                            </div>
                        </div>
                ...
                <form onSubmit={handleSubmit}>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription className="mb-1">
                            Tindakan ini tidak dapat dibatalkan. Ini akan menghapus kehadiran secara permanen
