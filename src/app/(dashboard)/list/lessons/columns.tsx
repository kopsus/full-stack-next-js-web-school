...
export const columns: ColumnDef<LessonWithDetails>[] = [
  {
    accessorKey: "name",
    header: () => <div className="truncate">Nama Pelajaran</div>,
    cell: ({ row }) => (
  ...
  {
    accessorKey: "day",
    header: () => <div className="truncate">Hari</div>,
    cell: ({ row }) => (
      <div className="flex flex-col min-w-0">
        <p className="text-xs md:text-sm">{dayNames[row.original.day]}</p>
      </div>
    ),
  },
  {
    id: "time",
    header: () => <div className="truncate">Waktu</div>,
    cell: ({ row }) => (
  ...
  {
    id: "subject",
    header: () => <div className="truncate">Mata Pelajaran</div>,
    cell: ({ row }) => (
      <div className="truncate">
        <span className="text-xs md:text-sm">{row.original.subject.name}</span>
      </div>
    ),
  },
  {
    id: "teacher",
    header: () => <div className="truncate">Guru</div>,
    cell: ({ row }) => {
      const teacher = row.original.allTeachers.find(
        (t) => t.id === row.original.teacherId
      );
      return (
        <div className="truncate text-xs md:text-sm">
          {teacher ? `${teacher.first_name} ${teacher.last_name}` : "Tidak ada"}
        </div>
      );
    },
  },
  {
    id: "class",
    header: () => <div className="truncate">Kelas</div>,
    cell: ({ row }) => (
      <div className="truncate">
        <span className="text-xs md:text-sm">{row.original.class.name}</span>
      </div>
    ),
  },
  {
          ...
          {row.original.roleLogin === "ADMIN" && (
            <>
              <ButtonEditLesson
                ...
                teachers={row.original.allTeachers}
                classes={row.original.allClasses}
              />
            ...
            <div className="mt-4 space-y-4 text-left">
              ...
              <div>
                ...
                <span>
                  ...
                  {dayjs(data.endTime).format("HH:mm")} WIB
                </span>
              </div>
              <div>
        ...
        <form onSubmit={handleSubmit}>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="mb-1">
              Tindakan ini tidak dapat dibatalkan. Ini akan menghapus pelajaran
