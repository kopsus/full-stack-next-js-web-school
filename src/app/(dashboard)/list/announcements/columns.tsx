...
export const columns: ColumnDef<AnnouncementWithDetails>[] = [
  {
    accessorKey: "title",
    header: () => <div className="truncate">Judul</div>,
    cell: ({ row }) => (
  ...
  },
  {
    ...
    header: () => <div className="truncate">Deskripsi</div>,
    cell: ({ row }) => (
  ...
  {
    accessorKey: "date",
    header: () => <div className="truncate">Tanggal</div>,
    cell: ({ row }) => (
      ...
      <div className="min-w-[100px]">
        <span className="text-xs md:text-sm truncate">
          {row.original.classId
            ? row.original.allClasses.find(
                (cls) => cls.id === row.original.classId
              )?.name
            : "Umum"}
        </span>
      </div>
          ...
          <AlertDialogDescription asChild>
            <div className="mt-4 space-y-4 text-left">
              ...
              <div>
                ...
                <span>
                  {data.classId
                    ? data.allClasses.find((cls) => cls.id === data.classId)
                        ?.name
                    : "Umum"}
                </span>
              </div>
        ...
        <form onSubmit={handleSubmit}>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="mb-1">
              Tindakan ini tidak dapat dibatalkan. Ini akan menghapus pengumuman
