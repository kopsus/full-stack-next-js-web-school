...
export function DataTable<TData, TValue>({
    ...
    const filteredData = React.useMemo(() => {
        return data.filter((row: any) => {
            ...
            const matchesClass = classFilter === "All" || 
                (classFilter === "Umum" ? row.classId === null : 
                row.allClasses.find((cls: any) => cls.name === classFilter)?.id === row.classId)

            const announcementDate = new Date(row.date)
            ...
            if (dateFilter === "today") {
                ...
                matchesDate = announcementDate.toDateString() === yesterday.toDateString()
            } else if (dateFilter === "tomorrow") {
                matchesDate = announcementDate.toDateString() === tomorrow.toDateString()
            } else if (dateFilter === "lastWeek") {
                matchesDate = announcementDate >= lastWeekStart && announcementDate < today
            } else if (dateFilter === "nextWeek") {
                matchesDate = announcementDate > today && announcementDate <= nextWeekEnd
    ...
    const uniqueClasses = React.useMemo(() => {
        ...
        data.forEach((row: any) => {
            if (row.allClasses) {
                row.allClasses.forEach((cls: any) => {
                ...
                <div className="flex flex-col gap-2">
                    ...
                    <Select value={classFilter} onValueChange={setClassFilter}>
                        ...
                        <SelectContent>
                            ...
                            <SelectItem className="text-xs md:text-sm" value="Umum">Umum</SelectItem>
