...
export const columns: ColumnDef<ClassWithDetails>[] = [
  {
    accessorKey: "name",
    header: () => <div className="truncate">Nama Kelas</div>,
    cell: ({ row }) => (
  ...
  },
  {
    ...
    header: () => <div className="truncate">Kapasitas</div>,
    cell: ({ row }) => (
      <div className="flex flex-col min-w-0">
        <p className="text-xs md:text-sm">{row.original.capacity} siswa</p>
      </div>
    ),
  },
  {
    id: "grade",
    header: () => <div className="truncate">Tingkat</div>,
    cell: ({ row }) => (
      <div className="truncate">
        <span className="text-xs md:text-sm">{row.original.grade.level}</span>
      </div>
    ),
  },
  {
    id: "supervisor",
    header: () => <div className="truncate">Wali Kelas</div>,
    cell: ({ row }) => (
      <div className="truncate">
        <span className="text-xs md:text-sm">
          {row.original.supervisor
            ? `${row.original.supervisor.first_name} ${row.original.supervisor.last_name}`
            : "-"}
        </span>
      </div>
    ),
              ...
              <ButtonEditClass
                ...
                defaultValues={{
                  ...
                  supervisorId: row.original.supervisorId
                    ? Number(row.original.supervisorId)
                    : undefined,
                }}
                teachers={row.original.allTeachers}
                grades={row.original.allGrades}
              />
