...
export function DataTable<TData, TValue>({
    ...
    const filteredData = React.useMemo(() => {
        return data.filter((row: any) => {
            ...
            const matchesSearch = (
                ...
                (row.class?.name || '').toLowerCase().includes(searchValue) ||
                (row.parent?.first_name || '').toLowerCase().includes(searchValue) ||
                (row.parent?.last_name || '').toLowerCase().includes(searchValue)
            )

            const matchesBloodType = bloodType === "All" || row.blood_type === bloodType
            const matchesGender = gender === "All" || row.sex === gender
            const matchesClass = selectedClass === "All" || row.class?.name === selectedClass

            return matchesSearch && matchesBloodType && matchesGender && matchesClass
        })
    }, [data, searchTerm, bloodType, gender, selectedClass])
    ...
    return (
        <div className="flex flex-col gap-2">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-start w-full my-2">
                <div className="flex flex-col gap-2">
                    <Label htmlFor="search">Pencarian</Label>
                    <Input
                        id="search"
                        placeholder="Cari berdasarkan Nama, Username, Kelas, Orang Tua..."
                        value={searchTerm}
                    ...
                    <Select value={selectedClass} onValueChange={setSelectedClass}>
                        ...
                        <SelectContent>
                            ...
                            {classes.map((classItem: Class) => (
                                <SelectItem key={classItem.id} className="text-xs md:text-sm" value={classItem.name}>
                                    {classItem.name}
                                </SelectItem>
                            ))}
                        </SelectContent>
