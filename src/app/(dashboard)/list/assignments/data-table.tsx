...
export function DataTable<TData, TValue>({
    ...
    const filteredData = React.useMemo(() => {
        return data.filter((row: any) => {
            ...
            const matchesSubject = subject === "All" || row.lesson.subject.name === subject
            const matchesClass = selectedClass === "All" || row.lesson.class.name === selectedClass
    ...
    const subjects = React.useMemo(() => {
        const uniqueSubjects = new Set(data.map((row: any) => row.lesson.subject.name))
        return Array.from(uniqueSubjects)
    }, [data])

    const classes = React.useMemo(() => {
        const uniqueClasses = new Set(data.map((row: any) => row.lesson.class.name))
                    ...
                    <Select value={subject} onValueChange={setSubject}>
                        ...
                        <SelectContent>
                            ...
                            {subjects.map((subject) => (
                                <SelectItem key={subject} className="text-xs md:text-sm" value={subject}>
                                    {subject}
                                </SelectItem>
                            ))}
                        </SelectContent>
